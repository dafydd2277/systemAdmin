...
Here are a couple commands to handle the settings changes. You can
find `fn_archive()` [here][functions].

```bash
df_sysctl=/etc/sysctl.d/10-389-ds.conf
df_limits=/etc/security/limits.d/10-nobody.conf

if [ -f "${df_sysctl}"]
then
  fn_archive ${df_sysctl}
fi

echo "net.ipv4.tcp_keepalive_time = 300" >>${df_sysctl}


if [ -f "${df_limits}"]
then
  fn_archive ${df_limits}
fi
...
[functions]: https://github.com/dafydd2277/systemAdmin/blob/master/scripting/functions


## Set up the `.inf` file.

The 389-ds `setup-ds-admin.pl` setup script can take an answer file in
`.inf` format. The file is described in [Section 4.5][sec45] for the
RHDS Installation Guide. This is particularly handy in development
situations, where you can blow away your 389-ds installation and
recreate it. The options are described in [Section 1.4][sec14].

Alternately, you can use `setup-ds-admin.pl --keepcache` to save a
`.inf` file from the first time you execute the script. See Table 1.1
in [Section 1.3][sec13] of the RHDS Installation Guide.

**IMPORTANT**: clear-text passwords are kept in this file. So, let's
keep it in `root`'s home directory, along with the passphrase files,
themselves.

One other trick: note that I'm using `fn_randomChars()` from
the [functions file][functions]. This will create an arbitrary
length password string with characters from the set `0-9a-zA-Z`.
Using this method will be hard if you just to run
`setup-ds-admin.pl` by hand. My suggestion for that case is that you
have a second terminal window open to allow you to copy and paste the
passphrases into the appropriate places. If you choose to do the
interactive installation, the contents of `${df_dsadmin_passphrase}`
...
fn_randomChars  > ${df_dsadmin_passphrase}
chown root:root ${df_dsadmin_passphrase}
chmod 0400 ${df_dsadmin_passphrase}


fn_randomChars  > ${df_dirmgr_passphrase}
...
These files will be used repeatedly, later on. So, they're separate.
Here's the rest of the manually created .inf file.
